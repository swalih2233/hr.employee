...
def employee_dashboard(request):
    ...
    for leave_request in leave_requests:
        leave_request.calculated_duration = leave_request.calculate_working_days()
...
def apply_leave(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if is_half_day and half_day_session not in ('AM', 'PM'):
...
def leavelist(request):
    ...
    for instance in instances:
        instance.leave_duration = instance.calculate_working_days()
...
def withdraw_leave(request, id):
    ...
    leave_request = get_object_or_404(LeaveRequest, id=id, employee=current_employee)

    # Allow withdrawal for approved leaves only
    ...
    if request.method == 'POST':
        ...
        leave_request.withdrawal_subject = subject
        leave_request.withdrawal_reason = reason
        leave_request.withdrawal_requested_date = timezone.now()
        leave_request.save()

        # Notification logic
        try:
            # Send to manager and CC founder
            if current_employee.manager:
                manager_email = current_employee.manager.user.email
                # We'll need to handle 'withdrawal_requested' in send_leave_notification
                send_leave_notification(request, leave_request, 'withdrawal_requested', manager_email, cc_founder=True)
            
            # Send to founder if they exist as a direct link
            elif current_employee.founder:
                founder_email = current_employee.founder.user.email
                send_leave_notification(request, leave_request, 'withdrawal_requested', founder_email)

            messages.success(request, "Withdrawal request submitted successfully.")
        except Exception as e:
            messages.warning(request, f"Withdrawal requested but notification failed: {str(e)}")

        return redirect(reverse('employe:leavelist'))

    return render(request, 'employe/withdraw_leave.html', {
        'leave_request': leave_request,
        'employe': current_employee
    })
