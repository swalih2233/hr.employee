...
class ManagerAdminForm(forms.ModelForm):
    ...
    class Meta:
        model = Manager
        fields = [
            'department', 'designation', 'manager_id', 'founder', 'date_of_joining', 'employment_Type', 
            'reporting_manager', 'work_location', 'image', 
...
class UnifiedLeaveRequestForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['subject', 'start_date', 'end_date', 'leave_type', 'description', 'file', 'is_half_day', 'half_day_session']
        widgets = {
            'start_date': forms.DateInput(attrs={'type': 'date', 'class': 'half-day-start mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500'}),
            'end_date': forms.DateInput(attrs={'type': 'date', 'class': 'half-day-end mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500'}),
            'description': forms.Textarea(attrs={'rows': 4}),
            'is_half_day': forms.CheckboxInput(attrs={'class': 'half-day-toggle rounded border-gray-300'}),
            'half_day_session': forms.HiddenInput(),
        }
    ...
    def clean(self):
        ...
        if is_half_day:
            if cleaned_data.get('start_date') != cleaned_data.get('end_date'):
                raise forms.ValidationError('Half-day leave must be for a single date (same start and end date).')
            if cleaned_data.get('half_day_session') not in ('AM', 'PM'):
                raise forms.ValidationError('Please select Morning or Afternoon for half-day leave.')
        else:
            cleaned_data['half_day_session'] = None
        return cleaned_data
